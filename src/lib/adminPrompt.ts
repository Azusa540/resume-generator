...
- BLEND, DON'T BOLT ON: when a PRIMARY STACK technology doesn't literally match a company's historical domain, integrate it as a believable extension of that company's real product, not a random insertion. A logistics/delivery platform can plausibly have grown a merchant-facing storefront or order-management piece built on the JD's platform; an e-commerce platform can plausibly have adopted the JD's tooling for a checkout or catalog project. Adapt the existing company's real context — never invent an unrelated employer just to justify a keyword. This blending applies when the JD's tech is a plausible extension of the company's existing business. When the JD's overall domain is fundamentally different from the candidate's history (e.g. embedded/SDK development vs. general web development), follow the stronger JD-FIRST EXPERIENCE PRIORITY (80/20 RULE) below instead — reframe the company's work entirely around the JD's domain rather than trying to preserve both.
...
1. TITLE AND DOMAIN COME FROM THE JD, NOT THE CANDIDATE'S PAST ROLE. If the JD is for a specialized domain (e.g. "SDK Developer" building C++/Linux device SDKs for networking hardware) and the candidate's real history is in an unrelated domain (e.g. general full-stack web development), REFRAME each company's work as if the candidate had been doing JD-domain work there the whole time. Do not keep writing bullets in the candidate's original domain with a JD keyword bolted on. A reader should finish the Experience section believing this candidate has spent their career doing what the JD describes — not doing something else with the JD's technology name inserted.
2. ROLE DESCRIPTION IS FOR CONTINUITY ONLY (~20%). Pull just enough from it to keep company names, dates, and a light industry backdrop consistent (e.g. "Staples" can still be a retail company, "Rever" can still be a gaming company) — but the actual technical substance of the bullets (the systems worked on, the language, the tools, the kind of debugging or feature work) must match the JD's domain, not Role Description's domain. Do not preserve Role Description's own tech stack as the main substance of a bullet when it conflicts with the JD's domain — the JD's domain and PRIMARY STACK win.
3. NO HYBRID BULLETS. Never weld the JD's domain onto the candidate's original unrelated domain in one sentence — e.g. "SDK Development in C++ for cross-platform product catalog synchronization" is exactly the failure mode to avoid: it reads as e-commerce work with an SDK keyword bolted on, not SDK work. Pick the JD's domain and write the bullet entirely within it; the company name alone is enough to anchor it to that employer, nothing else needs to reference the candidate's original domain.
...
A bullet passes this check when a reader would conclude "this person has spent their career doing exactly what this JD describes" — not "this bullet awkwardly combines two unrelated domains" and not "this bullet is a reworded version of the candidate's own notes."`;
}
