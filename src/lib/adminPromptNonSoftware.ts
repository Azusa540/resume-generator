...
- BLEND, DON'T BOLT ON: when a PRIMARY DOMAIN STACK item doesn't literally match a company's historical domain, integrate it as a believable extension of that company's real operations, not a random insertion — this applies for SAME/ADJACENT overlap (see DOMAIN OVERLAP CALIBRATION below). For DISTANT overlap, follow DEFENSIBLE ADJACENT FRAMING instead — the PRIMARY DOMAIN STACK is pre-filtered by the DISTANT OVERLAP EXCEPTION above, so what remains should already be plausible to frame this way.
...
- DOMAIN TOOL DENSITY GUIDANCE: Most bullets should reference at least 1 specific named tool, platform, standard, or methodology from the Skills list — that's the default. A rare genuine process/mentorship/team bullet can skip a named tool if it is truly about people or process, not tools — don't force a tool name into a sentence that isn't about one.
...
Example note: "Intentionally omitted: ISO 14001, Six Sigma | Added beyond JD: AutoCAD, Primavera P6 | Bullets reordered within each experience block."
...
- DISTANT: do NOT claim direct ownership or hands-on authorship of the JD's specialized core work (e.g. do not claim the candidate personally performed clinical procedures or aerospace certification testing if their real background is retail operations). Use DEFENSIBLE ADJACENT FRAMING instead: describe realistic peripheral involvement that someone with the candidate's ACTUAL background could plausibly have had — coordinating with, supporting, reporting on, tracking compliance for, or being trained alongside specialists in the JD's domain, anchored in the candidate's real skills (project coordination, quality processes, reporting, general operations, vendor management). The DISTANT OVERLAP EXCEPTION above already keeps the PRIMARY DOMAIN STACK limited to items that can be framed this way.
