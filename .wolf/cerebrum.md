...
- **Production infra (VPS `104.207.74.198`, AlmaLinux 9):** app is NOT run via the repo's Dockerfile/docker-compose.yml — those exist but are unused. Real setup: `/var/www/resume-generator` is a plain git checkout, run via **PM2** (`pm2 restart resume-generator`, process name exactly `resume-generator`, `npm run start` under Node 22 in fork mode), fronted by **nginx** (`/etc/nginx/conf.d/resume.syntra.best.conf`) terminating TLS (Certbot-managed cert) and proxying 443→127.0.0.1:3000. `CHROMIUM_PATH` is empty in the VPS `.env` — Puppeteer uses its own downloaded Chrome at `~/.cache/puppeteer`, not a system package, so deploys must NOT set `PUPPETEER_SKIP_DOWNLOAD` there (unlike CI).
- **CD:** `.github/workflows/deploy.yml`, fires via `workflow_run` on `CI` completing for `master` (green only), or `workflow_dispatch` with an optional `ref` for manual redeploy/rollback. Deploys over SSH using a dedicated ed25519 key (GitHub secrets `DEPLOY_SSH_KEY`/`DEPLOY_HOST`/`DEPLOY_USER`) added to the VPS's root `authorized_keys` — the VPS previously only had password auth. Auto-rolls back to the previous commit on a failed post-deploy health check.
...
- **[2026-08-18] Phase 2 CD: SSH+PM2 deploy, not Docker**
  - STATUS.md had left "SSH+docker-compose vs GHCR" as an open decision. Neither was right: inspecting the live VPS showed production actually runs via PM2 + nginx, with the repo's Dockerfile/docker-compose.yml unused. Built the deploy workflow to match reality (PM2 restart) instead of introducing a container-based deploy the app doesn't otherwise use.
  - Chose `workflow_run` (listening for `CI` completing on `master`) over duplicating lint/typecheck/build inside deploy.yml — avoids redundant work and guarantees deploy only fires on a green CI run.
  - Added automatic rollback on failed post-deploy health check (reset to previous commit, rebuild, restart) since this is unattended production infra with no one watching each deploy.
  - VPS only had password auth; bootstrapped a dedicated deploy-only ed25519 key via one-off script (password used once, never stored), pushed the private key straight to GitHub Actions secrets via the API, then deleted the local copy — nothing sensitive left on disk afterward.
